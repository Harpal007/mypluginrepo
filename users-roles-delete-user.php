...
function delete_user_add_toplevel_menu() {
	...
	add_menu_page(
		esc_html__('Users and Roles: Delete User', 'useraccess'),
		esc_html__('Delete User', 'useraccess'),
		'manage_options',
		'useraccess',
...
function delete_user_display_settings_page() {
	...
	<div class="wrap">
		...
		<form method="post">
			<h3><?php esc_html_e( 'Delete User', 'useraccess' ); ?></h3>
			<p>
				<label for="email">
			...
			<input type="submit" class="button button-primary" value="<?php esc_html_e( 'Delete User', 'useraccess' ); ?>">
		</form>
	</div>
	...
	if ( isset( $_POST['useraccess-nonce'] ) && wp_verify_nonce( $_POST['useraccess-nonce'], 'useraccess-nonce' ) ) {
		...
		if ( is_numeric( $user_id ) ) {
			...
			if ( $result ) {
				...
				$result = __( 'The user has been deleted.', 'useraccess' );

			} else {

				$result = __( 'Error: user not deleted.', 'useraccess' );
			...
			// user not found
			$result = __( 'Error: user not found.', 'useraccess' );

		}

		// set the redirect url
		$location = admin_url( 'admin.php?page=useraccess&result='. urlencode( $result ) );
		...
		if ( isset( $_GET['result'] ) ) {
			...
			if ( strpos( $_GET['result'], 'Error' ) === false ) : ?>
				...
				<div class="notice notice-success is-dismissible">
					<p><strong><?php esc_html_e('User deleted.', 'useraccess'); ?></strong></p>
				</div>
