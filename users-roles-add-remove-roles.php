<?php
/*
Plugin Name: Add and Remove Roles
...
function users_roles_examples_add_toplevel_menu() {
	...
	add_menu_page(
		esc_html__('Users and Roles: Add & Remove Roles', 'useraccess'),
		esc_html__('Add & Remove Roles', 'useraccess'),
		'manage_options',
...
function users_roles_examples_display_settings_page() {
	...
	<div class="wrap">
		...
		<p><?php esc_html_e( 'This plugin demo shows all user roles and capabilities for the site.', 'useraccess' ); ?></p>
...
function users_roles_examples_display_results() {
	...
	foreach ( $roles as $role ) {
		...
		if ( $all_caps ) {
			...
			echo '<h3>'. $role_name .' '. esc_html__( 'capabilities', 'useraccess' ) .'</h3>';
...
/*
	...
	The plugin functions useraccess_add_role() and useraccess_remove_role()
	...
	So it is better for performance to run useraccess_add_role() and
	useraccess_remove_role() only on plugin activation, using the hook
	...
	USAGE:
		...
		Add a role on plugin activation
			...
			1. Remove the two slashes from useraccess_add_role()
			2. Remove the add_action() for useraccess_add_role()

		Remove a role on plugin activation

			1. Remove the two slashes from useraccess_remove_role()
			2. Remove the add_action() for useraccess_remove_role()
